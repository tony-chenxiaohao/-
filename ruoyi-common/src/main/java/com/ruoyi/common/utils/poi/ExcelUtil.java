        ...
        for (int i = startNo; i < endNo; i++)
        {
            ...
            if (isSubList())
            {
                if (isSubListValue(vo))
                {
                    ...
                    List<Integer> collect = fields.stream().map(f -> {
                        Field field = (Field) f[0];
                        if (Collection.class.isAssignableFrom(field.getType()) && CollUtil.isNotEmpty(getListCellValue(vo, getSubMethod(field.getName(), clazz)))) {
                            return getListCellValue(vo, getSubMethod(field.getName(), clazz)).size();
                        }
                        return 0;
                    }).collect(Collectors.toList());
                    Integer max = collect.stream().max(Comparator.comparingInt(o -> o)).orElse(0);
//                    this.subMergedFirstRowNum = this.subMergedFirstRowNum + max;
                    this.subMergedLastRowNum = this.subMergedLastRowNum + max;
                }
                else
                {
            ...
            for (Object[] os : fields)
            {
                ...
                Excel excel = (Excel) os[1];
                if (Collection.class.isAssignableFrom(field.getType())) {
                    subList = getListCellValue(vo, getSubMethod(field.getName(), clazz));
                }
     ...
     * 获取集合的值
     */
    public Collection<?> getListCellValue(Object obj, Method subMethod) {
        Object value;
        try {
            value = subMethod.invoke(obj, new Object[]{});
        }
